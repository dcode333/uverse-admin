...
function Badges(props) {
    ...
    const { data: items, isError, isLoading, refetch, isRefetching } = useBadges(authToken)
    const { mutateAsync, isPending } = useDeleteBadgeItem()
    ...
    const handleDelete = async () => {
        try {
            ...
            await mutateAsync({ token: authToken, badgeId: deleteConfirmationItem.id })
            refetch()
            ...
            >
                <Box sx={{
                    ...
                    <Typography id="modal-modal-title"
                        variant="h6"
                        component="h2">
                        {` Do you want to delete "${deleteConfirmationItem.title}" Badge ?`}
                    </Typography>
                    <Box sx={{ display: 'flex' }}>
                        <LoadingButton
                            variant="contained"
