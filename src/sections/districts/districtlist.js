...
import { useDistricts } from 'src/api/districts/usegetDistricts';
import Link from 'next/link';

function CheckIn(props) {

    const { authToken } = props;
    const { data: items, isError, isLoading, refetch, isRefetching } = useDistricts({ token: authToken })
    // const { mutateAsync, isPending } = useDeleteCheckinItem()
    ...
    const handleDelete = async () => {
        try {
            ...
            // await mutateAsync({ token: authToken, checkinId: deleteConfirmationItem.id })
            // refetch()
                    ...
                    items?.results.map((item, index) => (
                        <Grid
                            ...
                            <Card sx={{ backgroundColor: 'neutral.3000', m: 1, borderRadius: 1, position: 'relative' }}>
                                ...
                                <Link href={`/districts/${item.id}`}
                                    style={{ textDecoration: 'none' }}>
                                    <CardMedia
                                        sx={{ height: 200 }}
                                        image={item.media.length > 0  ? item.media[0]?.media : "/assets/errors/error-404.png"}
                                        title="Check-in"
                                    />
                                    <CardContent>
                                        <Typography
            ...
            >
                <Box sx={{
                    ...
                    <Typography id="modal-modal-title"
                        variant="h6"
                        component="h2">
                        {` Do you want to delete "${deleteConfirmationItem.title}" District ?`}
                    </Typography>
                    <Box sx={{ display: 'flex' }}>
                        <LoadingButton
                            variant="contained"
