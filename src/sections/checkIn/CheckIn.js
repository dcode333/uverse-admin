...
import { useCheckin, useDeleteCheckinItem } from 'src/api/checkin/useCheckin';
import Link from 'next/link';

function CheckIn(props) {

    const { authToken } = props;
    const { data: items, isError, isLoading, refetch, isRefetching } = useCheckin(authToken)
            ...
            >
                <Box sx={{
                    ...
                    <Typography id="modal-modal-title"
                        variant="h6"
                        component="h2">
                        {` Do you want to delete "${deleteConfirmationItem.title}" Checkin ?`}
                    </Typography>
                    <Box sx={{ display: 'flex' }}>
                        <LoadingButton
                            variant="contained"
