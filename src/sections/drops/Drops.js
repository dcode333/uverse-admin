...
import { useDeleteDropItem } from 'src/api/drops/usedeleteDrops';
import { useDrops } from 'src/api/drops/usegetDrops';
import Link from 'next/link';

function Drops(props) {

    const { authToken } = props;
    const { data: items, isError, isLoading, refetch, isRefetching } = useDrops({ token: authToken })
    const { mutateAsync, isPending } = useDeleteDropItem()
    ...
    const handleDelete = async () => {
        try {
            ...
            await mutateAsync({ token: authToken, dropId: deleteConfirmationItem.id })
            refetch()
                        ...
                        <Grid
                            ...
                            <Card sx={{ backgroundColor: 'neutral.3000', m: 1, borderRadius: 1, position: 'relative' }}>
                                <LoadingButton
                                    ...
                                    onClick={() => handleModalOpen({ id: item.id, title: item.name })}
                                >
                                    Delete
                                </LoadingButton>
            ...
            >
                <Box sx={{
                    ...
                    <Typography id="modal-modal-title"
                        variant="h6"
                        component="h2">
                        {` Do you want to delete "${deleteConfirmationItem.title}" Drop ?`}
                    </Typography>
                    <Box sx={{ display: 'flex' }}>
                        <LoadingButton
                            variant="contained"
